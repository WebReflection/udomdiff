  ...
  var index = (function (parentNode, a, b, get, before) {
    ...
    while (aStart < aEnd || bStart < bEnd) {
      ...
      if (aEnd === aStart) {
        ...
        var nodes = [];
        while (bStart < bEnd) nodes.push(get(b[bStart++], 1));
        if (node) {
          nodes.push(node);
          node.replaceWith.apply(node, nodes);
        } else {
          parentNode.append.apply(parentNode, nodes);
        }
      }
        ...
        if (map.has(a[aStart])) {
          ...
          if (bStart < index && index < bEnd) {
            ...
            if (sequence > index - bStart) {
              ...
              var _nodes = [];
              while (bStart < index) _nodes.push(get(b[bStart++], 1));
              _nodes.push(_node2);
              _node2.replaceWith.apply(_node2, _nodes);
            }
